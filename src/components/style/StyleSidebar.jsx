...
const Toggle = ({ label, description, checked, onChange }) => (
    <div className="flex items-start justify-between gap-4 py-3">
        ...
        <button
            ...
            className={`relative inline-flex h-[22px] w-10 flex-shrink-0 rounded-full border-2
        border-transparent cursor-pointer transition-colors duration-200 ease-in-out
        ${checked ? 'bg-blue-500' : 'bg-slate-200'}`}
        >
            <span className={`pointer-events-none inline-block h-[18px] w-[18px] rounded-full
...
const FONT_OPTIONS = [
    { value: 'serif', label: 'Classic Serif', preview: '"Times New Roman", Times, serif', sub: 'Traditional & ATS-Safe', sample: 'Abc' },
    { value: 'lora', label: 'Elegant Lora', preview: '"Lora", Georgia, serif', sub: 'Refined & Distinguished', sample: 'Abc' },
    { value: 'sans', label: 'Modern Sans', preview: '"Inter", Arial, sans-serif', sub: 'Clean & Contemporary', sample: 'Abc' },
    { value: 'mono', label: 'Technical Mono', preview: '"Courier New", monospace', sub: 'Developer-Friendly', sample: 'Abc' },
];
...
const FontCard = ({ option, selected, onClick }) => (
    ...
    >
        ...
        transition-colors ${selected ? 'bg-blue-100 text-blue-700' : 'bg-slate-100 text-slate-500'}`}
      ...
      transition-all duration-150 ${selected ? 'bg-blue-500 opacity-100' : 'opacity-0'}`}>
...
const HEADING_STYLES = [
    { value: 'underline', label: 'Underline', desc: 'Classic rule below' },
    { value: 'overline', label: 'Overline', desc: 'Bold rule above' },
    { value: 'sidebar', label: 'Sidebar', desc: 'Colour left-bar' },
    { value: 'minimal', label: 'Minimal', desc: 'No rule, spaced' },
];

const HeadingStylePicker = ({ value, onChange }) => (
    <div className="grid grid-cols-2 gap-2">
        {HEADING_STYLES.map((opt) => (
          ...
          ${value === opt.value
            ...
            >
                <p className={`text-xs font-bold ${value === opt.value ? 'text-blue-600' : 'text-slate-600'}`}>
                    {opt.label}
                </p>
...
const PAPER_COLORS = [
    ...
    { value: 'warm', label: 'Warm White', hex: '#fafaf7' },
    { value: 'cream', label: 'Cream', hex: '#fdf8f0' },
];

const PaperColorPicker = ({ value, onChange }) => (
    <div className="flex gap-2">
        {PAPER_COLORS.map((p) => (
            <button
                ...
                title={p.label}
                className={`flex-1 flex flex-col items-center gap-1.5 p-2 rounded-xl border-2
          transition-all duration-150
          ${value === p.value ? 'border-blue-400' : 'border-slate-150 hover:border-slate-300'}`}
            >
                <span
                    className="w-8 h-8 rounded-lg border border-slate-200 shadow-inner"
                    style={{ background: p.hex }}
                />
                <span className={`text-[10px] font-semibold ${value === p.value ? 'text-blue-600' : 'text-slate-500'}`}>
                    {p.label}
                </span>
            </button>
        ))}
...
const StyleSidebar = () => {
    ...
    return (
        <div className="pb-24">
            ...
            <SectionLabel icon="⠿" title="Section Order" />
            <SectionOrderPanel />
            ...
            <RangeSlider label="Body Text Size"
                value={styleConfig.globalFontSize} min={10} max={16}
                onChange={(v) => updateStyle('globalFontSize', v)} />

            <RangeSlider label="Section Heading Size"
                value={styleConfig.headingFontSize} min={9} max={16}
                onChange={(v) => updateStyle('headingFontSize', v)} />

            <RangeSlider label="Name / Title Size"
                value={styleConfig.nameSize} min={20} max={48}
                onChange={(v) => updateStyle('nameSize', v)} />

            <RangeSlider label="Line Height"
                value={styleConfig.lineHeight} min={1.0} max={2.0} step={0.05} unit="×"
                onChange={(v) => updateStyle('lineHeight', v)} />

            <RangeSlider label="Section Spacing"
                value={styleConfig.sectionSpacing} min={8} max={32} unit="px"
            ...
            <div className="space-y-2">
                {FONT_OPTIONS.map((opt) => (
                    <FontCard key={opt.value} option={opt}
            ...
            {/* ── Heading Style ────────────────────────────────────────── */}
            <SectionLabel icon="≡" title="Heading Style" />
            <HeadingStylePicker
                value={styleConfig.headingStyle}
                onChange={(v) => updateStyle('headingStyle', v)}
            />
            ...
            <div className="mt-3 mb-1">
                ...
                <div className="flex gap-2">
                    {[1, 1.5, 2].map((t) => (
                        <button
                            key={t}
                            onClick={() => updateStyle('dividerThickness', t)}
                            className={`flex-1 py-2 rounded-lg text-xs font-bold border-2 transition-all
                ${styleConfig.dividerThickness === t
            ...
            <div className="mb-4">
                ...
                <div className="flex items-center gap-2 mb-3">
                    <div className="relative w-9 h-9 rounded-lg overflow-hidden border border-slate-200 shadow-sm cursor-pointer flex-shrink-0">
                        <div className="w-full h-full" style={{ backgroundColor: styleConfig.linkColor }} />
                        <input type="color" value={styleConfig.linkColor}
            ...
            {/* Paper Color */}
            <div className="mb-1">
                <label className="text-sm font-medium text-slate-600 block mb-2.5">Paper Color</label>
                <PaperColorPicker
                    value={styleConfig.paperColor}
                    onChange={(v) => updateStyle('paperColor', v)}
                />
            </div>
            ...
            <div className="bg-slate-50 rounded-xl px-3 divide-y divide-slate-100">
                ...
                <Toggle
                    label="Show Summary"
                    description="Professional summary below your name"
                    checked={styleConfig.showSummary}
                    onChange={(v) => updateStyle('showSummary', v)}
                />
            </div>
          ...
          hover:border-red-300 hover:text-red-400 hover:bg-red-50 transition-all duration-200 group"
            >
